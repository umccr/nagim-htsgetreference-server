...
class HtsgetStack(core.Stack):
    def __init__(
            ...
            dns_record_name: str,
            service_count: int = 1,
            service_memory: int = 512,
            service_cpu: int = 256,
            **kwargs,
    ) -> None:
        """
        A stack that spins up a Fargate cluster running htsget ref (htsget S3 with NAGIM auth) with an ALB in-front.
        ...
        asset = ecrassets.DockerImageAsset(self, "HtsgetImage",
                                           directory="server"
                                           )
